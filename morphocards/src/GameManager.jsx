    ...
    if (actualRound === GLOBAL_ROUND) {
      ...
      return (
        <EndPage
          ...
          rounds={rounds}
          title={modalTitle}
          goToMenu={openMainMenu}
          restartGame={startNewGame}
          isLogged={props.isLogged}
          time={gameBarRef.current.getTimeComponent().getTime()}
          timeValue={gameBarRef.current.getTimeComponent().getSeconds()}
        />
      );
    } else {
      // ROUND
  ...
  const appWin = () => {
    ...
    setButtons(
      <div className="py-3 grid gap-3">
        <Button
          onClick={nextRound}
          color="text-white hover:bg-indigo-700 bg-indigo-600 focus:ring-indigo-500"
        >
          {getNextButtonText()}
        </Button>
      </div>
  ...
  const appFail = (playerWord) => {
    ...
    setModalContent(
      <div className="grid gap-3">
        ...
        <WordDisplay
          word={playerWord}
          legend="Vous avez constitué le mot"
          say={say}
          align="center"
        />
      </div>
    ...
    setButtons(
      <div className="py-3 grid gap-3">
        <Button
          ...
          color="ring-red-200 text-white hover:bg-red-700 bg-red-600"
        >
          Réessayer
        </Button>
        <Button
          onClick={nextRound}
          color="text-white hover:bg-indigo-700 bg-indigo-600 ring-indigo-200"
        >
          {getNextButtonText()}
        </Button>
      </div>
